...
exports.default = (config) => {
            ...
            switch (type) {
                ...
                case 'notify':
                    for (const msg of newMessages) {
                        ...
                        // Cek dulu kalau list punya method upsert
                        if (typeof list.upsert === 'function') {
                          list.upsert(msg, 'append');
                        } else {
                          // Fallback kalau upsert ga ada
                          try {
                            // Jika list adalah array
                            if (Array.isArray(list)) {
                              // cari pesan yg sama berdasarkan key.id
                              const existingIndex = list.findIndex(m => m?.key?.id === msg?.key?.id);
                              if (existingIndex === -1) {
                                // belum ada, push baru
                                list.push(msg);
                              } else {
                                // sudah ada → bisa update / replace
                                list[existingIndex] = msg;
                              }
                            } 
                            // Jika list punya properti .array (beberapa impl store punya struktur .array)
                            else if (list.array && Array.isArray(list.array)) {
                              const existing = list.array.find(m => m?.key?.id === msg?.key?.id);
                              if (!existing) {
                                list.array.push(msg);
                              } else {
                                // replace
                                const idx = list.array.findIndex(m => m?.key?.id === msg?.key?.id);
                                list.array[idx] = msg;
                              }
                            }
                            // Jika list seperti Map
                            else if (list instanceof Map) {
                              list.set(msg.key.id, msg);
                            }
                            // Fallback umum kalau tipe lain
                            else {
                              // misalnya jika tipe object dengan id sebagai key
                              list[msg.key.id] = msg;
                            }
                          } catch (errFallback) {
                            console.error("⚠️ Fallback store upsert error:", errFallback);
                          }
                        }
    ...
    const fromJSON = (json) => {
      ...
      for (const jid in json.messages) {
        ...
        for (const msg of json.messages[jid]) {
          try {
            // ✅ Sudah object WebMessageInfo
            if (msg && typeof msg === "object" && msg.key && msg.message) {
              list.upsert(msg, "append");
              continue;
            }
    
            // ✅ Kalau undefined / kosong, skip
            if (!msg) continue;
    
            // ✅ Kalau string base64, ubah ke buffer
            if (typeof msg === "string") {
              try {
                const buf = Buffer.from(msg, "base64");
                const decoded = baileys_1.proto.WebMessageInfo.decode(buf);
                list.upsert(decoded, "append");
              } catch {
                console.warn("[⚠️ skip invalid base64 msg]");
              }
              continue;
            }
    
            // ✅ Kalau object dengan properti data (BufferJSON style)
            if (msg.data && Array.isArray(msg.data)) {
              try {
                ...
                console.warn("[⚠️ skip invalid data msg]");
              }
              continue;
            }
    
            // ❌ Format tidak diketahui
            console.warn("[⚠️ skip unknown msg format]", typeof msg);
          } catch (e) {
    ...
    return {
        ...
        writeToFile: (path) => {
            ...
            writeFileSync(path, JSON.stringify(toJSON(), getCircularReplacer(), 2));
        },
        readFromFile: (path) => {
            // require fs here so that in case "fs" is not available -- the app does not crash
